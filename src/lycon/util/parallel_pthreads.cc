...
struct work_load
{
    ...
    void set(const Range& range, const ParallelLoopBody& body, unsigned int nstripes)
    {
        m_body = &body;
        m_range = &range;
...
class ThreadManager
{
  public:
    ...
    {
        LYCON_SINGLETON_LAZY_INIT_REF(ThreadManager, new ThreadManager())
    }
...
const char ThreadManager::m_env_name[] = "LYCON_FOR_THREADS_NUM";
...
{
    unsigned int m_current_pos = LYCON_XADD(&m_parent->m_task_position, 1);
    ...
    while (m_current_pos < load.m_nstripes)
    {
        ...
        m_current_pos = LYCON_XADD(&m_parent->m_task_position, 1);
    }
}
...
ThreadManager::ThreadManager()
    : m_num_threads(0), m_task_complete(false), m_num_of_completed_tasks(0), m_pool_state(eTMNotInited)
{
    int res = 0;
...
void ThreadManager::run(const Range& range, const ParallelLoopBody& body, double nstripes)
{
    bool is_work_thread = m_is_work_thread.get()->value;

    if ((getNumOfThreads() > 1) && !is_work_thread && (range.end - range.start > 1) &&
        (nstripes <= 0 || nstripes >= 1.5))
    {
...
{
    unsigned int comp = LYCON_XADD(&m_num_of_completed_tasks, 1);
